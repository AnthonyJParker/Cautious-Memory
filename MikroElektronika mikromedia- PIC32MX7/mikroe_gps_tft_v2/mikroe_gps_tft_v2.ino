/*
     Logging GPS data on SD card via UART2 (mikroE Click GPS) i.e.(Serial1),
     on MikroE mikromedia MX7  
...
**** END SETUP.TXT****
 ...
 GPS        : mikroE Click GPS
************************************ working A Parker 2017 *******************
*/
# include <TinyGPS.h>                            // NOTE: include the TinyGPS library:
# include <DSPI.h>
# include <SPI.h>                                      // Required by Adafruit_STMPE610 touch library NOT USED
# include <SdFat.h>                                  // NOTE: use SDfat faster library
# include <Liberation.h>
//#include <SD.h>                                    // Required if reading BMP from SD
//#include <BMPFile.h>
# include <galaxy480.c>                         // image for startup splash screen
# include <SSD1963.h>
# include <Wire.h>
...
void buttons();
void screen1();                                         // NOTE: function to print values on tft lcd
void screen2();
void screen3();
void screenTouched();
...
int ledPinRed    = 49;                                // red 49, green 50, blue 51
int ledPinGreen = 50;
int ledPinBlue    = 51;
// *****************************************************************************************************************************
// setup file details
...
int NmeaInterval;
int x, y, z, oldx, oldy;                                      // NOTE: touch screen values
// ************************************************************************************************************
#define GPSBAUD 9600                             // NOTE:19200 // NOTE: Olimex GPS baudrate

const int chipSelect_SD_default = 6;      // NOTE: Change to suit your board // NOTE: Default SD chip select for MX7 6,
...
void setup() {
    ...
    Serial.println("GPS data logger version 0.01");
     ...
     if (! touch.begin(0x44)) {  // touch screen setup
   ...
   // tft.setRotation(1);
    ...
    tft.drawRGBA(1, 1, galaxy480_1, 480, 270, Color::Black);
...
//---------------------------------------If new data from GPS------------------------------------------
        if(newData == 1) {
             // digitalWrite(ledPinBlue, HIGH);                      //here because I may blink a LED
              ...
              //Serial.print(gps.hdop() == TinyGPS::GPS_INVALID_HDOP ? 0 : gps.hdop());          
               ...
               if(flagStart == 1) { // only on a good fix show screens
               screenTouched(); //  touch screen loop funtion
               }
//---------------------------------------first fix file start (run once only) -------------------------------

                          if(flagStart == 0) {  //check for first fix and write kml start flag header
                              tft.fillScreen(Color::Black); // clear startup screen on first good fix
...
//---------------------------------------end new data from GPS-------------------------------------------
     ...
     // digitalWrite(ledPinBlue, LOW);                                     //here because I may blink a LED
...
// *****************************************************************************
   void screen1(){ //NAV
      ...
      buttons();
      Serial.println("screen 2....DATE");
       //tft.fillScreen(Color::Black);                                 // from screenTouch function
      tft.setTextColor(Color::Green,Color::Black);   // NOTE must specify text colour and background colour
      tft.setFont(Fonts::Liberation30);
      //write to screen
      tft.setCursor(10, 15);
      tft.print("LAT: ");
...
// *****************************************************************************
  void screen3(){
      ...
      tft.setCursor(10, 15);
      tft.print("FILENAME: ");
      tft.setCursor(240, 25);                                       // to line up smaller font
...
// *****************************************************************************
    void buttons(){
       ...
       //   tft.fillRoundRect(x, y, width, height, 4, Color::Yellow);
        tft.fillRoundRect(20,  230, 110, 35, 4, Color::Red);
        tft.setTextColor(Color::Black, Color::Red);
        tft.setCursor(30, 238);
        tft.setFont(Fonts::Liberation16);
        tft.println("NAV");
       // button 2  
        tft.fillRoundRect(180, 230, 110, 35, 4, Color::Yellow);
        tft.setTextColor(Color::Black, Color::Yellow);
        tft.setCursor(190, 238);
        tft.setFont(Fonts::Liberation16);
        tft.println("DATE");
       // button 3
        tft.fillRoundRect(340, 230, 110, 35, 4, Color::Green);
        tft.setTextColor(Color::Black, Color::Green);
        tft.setCursor(350, 238);
        tft.setFont(Fonts::Liberation16);
        tft.println("FILE");  
}
// *****************************************************************************
// void squareButtons(int xp,int yp,int w, int h,char *string, char *bColor)
       // squareButtons(20  ,230,110,35,"NAV","Red");
       // squareButtons(180,230,110,35,"DATE","Yellow");
       // squareButtons(340,230,110,35,"FILE","Green");
// *****************************************************************************
  void squareButtons(int xp,int yp,int w, int h,char *string, char *bColor){
