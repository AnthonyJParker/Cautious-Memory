/*
     ...
     also has two tft print functions 1) for sailing, 2) for a vehicle
 
     SPECIAL NOTE: requires a file "SETUP.TXT" to be on the SD card
                   also reverted to the SD file library to display the BMP image
                   needs a BMP file "LR2.BMP" on the SD card sized to 320x480
**** SETUP.TXT*****
...
//ILI9481 tft(rs,wr,rd,cs,reset, d0,d1,d2,d3,d4,d5,d6,d7);
ILI9481 tft(A2, A1, A0, A3, 1, 8, 9, 2, 3, 4, 5, 6, 7);// NOTE: reset is on pin 1, Arduino shield pins

//*******CHANGE TIME ZONE HERE************
//const int TimeZone = 11;                                     // daylight savings time

  // in the format mdd
int DSTbegin[] = { //DST 2013 - 2025 Australia Victoria
  1006, 1005, 1004, 1002, 1001, 1007, 1006, 1004, 1003,1002, 1001, 1006, 1007};  
int DSTend[] = { //DST 2014 - 2026
  406, 405, 403, 402, 401, 407, 405, 404, 403, 402, 407, 406, 405}; 
...
static void UTCtoLocal();
void drawBMP();
void printTotft();                                         // NOTE: function to print values on tft lcd
void LRprintTotft();                                       // NOTE: tft display to suit a vehicle
...
float NMsoFar;                                               // NM distance travelled
...
File myFile, myFile1, myFile2, myFile3;                                // NOTE: myFile for the SETUP.TXT, myFile1 for the GPS00000.KML, myFile2 for the TXT00000.TXT
...
void setup() {
    ...
    tft.println("Land Rover Series III");
    //tft.println(" ");
    tft.setCursor(15, 275);
    tft.println(" swb 88in 1972");
  ...
  //For one second we parse GPS data and report some key values
      for(unsigned long start = millis(); millis() - start < 1000;) {
        while(Serial1.available()) {
          ...
          { newData = 1; }
   // add up the distance travelled for the KML footer
   // assume one RMC message per second and dont worry about missing messages
            NMsoFar += gps.f_speed_knots()/3600;     
        }
      }
//---------------------------------------If new data from GPS------------------------------------------
        if(newData == 1) {
              ...
              // find the length of the lat/lon
              char buf[256];
              ...
              sprintf(buf,"<Placemark><name>Finish %s (%s) Distance %f NM</name><Point><coordinates>%f,%f</coordinates></Point></Placemark>", st, sd, NMsoFar, flon, flat); // format the entry
              backStep = strlen(buf);
              Serial.print("Backstep length is: ");
              Serial.println(backStep);
              backStep = backStep + 64;                           // NOTE not sure how this adds up yet
...
//---------------------------------------first fix file start (run once only) -------------------------------
                      ...
                      /*
                               ...
                               delay(2000);
                      */
                              tft.fillScreen(Color::Black);                   // clear the display
 ...
 //----------------------------------------KML file write--------------------------------------------------
          if(KmlInterval == 0) {                                                //default if Kml time interval "equals 0 means write every second"
                ...
                if(myFile1) {
                  ...
                  updateSDfile(backStep);                                             //update KML file and footer includes km KmlMarkInterval 189 (191)
...
//  *****************************************************************************
 ...
 if (gpsYear%4 == 0) DaysAMonth[1] = 29; //leap year check
      ...
      if (gpsHour >= 24)
      {
        ...
        //sprintf(sdt, "%02d/%02d/%04d %02d:%02d:%02d ",gpsDay, gpsMonth, gpsYear, gpsHour, gpsMin, gpsSec);
        sprintf(sd, "%02d/%02d/%02d", gpsDay, gpsMonth, gpsYear);
        sprintf(st, "%02d:%02d:%02d", gpsHour, gpsMin, gpsSec);
...
// *****************************************************************************
// void LRprintTotft() Print to DX TFT display shield 3.5" 320x480
// *****************************************************************************
    void LRprintTotft() {
      //tft.setCursor(15, 15);
      ...
      //conv_float(gps.f_speed_kmph());
      conv_float(gps.f_speed_mph());  
      tft.setCursor(10, 70);
      tft.print("SPD: ");
      tft.print(sp);
      tft.println(" mph    ");   
...
// *****************************************************************************
// void drawBMP(); Print to DX TFT display shield 3.5" 320x480
// *****************************************************************************
void drawBMP(){

      File myFile3 = SD.open("LR2.BMP");
      BMPFile bmp(myFile3);
      bmp.draw(&tft, 0, 0);
      myFile3.close();
